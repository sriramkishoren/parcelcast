...
A weekend portfolio project demonstrating end-to-end parcel volume forecasting on real Walmart M5 data. Built to support an internal job application for a Senior Manager IC role on the Parcel Forecasting team at Walmart.

**Audience:** Pallavi (Director, Parcel Forecasting). She knows the WW14 reporting format, uses Traditional Error and WMAPE as primary/secondary metrics, and cares about FedEx HD contract compliance and OnTrac Tier 2 thresholds.
...
| HOBBIES | Channel: WFS (3P seller) |
| Units ÷ UPP | Package Volume |

UPP (Units Per Package) values: 1P ≈ 2.10 declining 3%/yr, WFS ≈ 1.33 static. The declining-1P-UPP insight matches the real WW14 report observation and is the project's signature analytical detail. Don't lose it.
...
### Naming
- `week_start` = the Saturday of a Walmart fiscal week (Sat–Fri)
...
- Prophet uses `seasonality_mode='multiplicative'` and the predefined `WALMART_HOLIDAYS` table in `src/models.py`
...
- Don't let it "improve" the domain mapping. The mapping is intentional and tied to the WW14 report.
- Don't let it switch from Prophet to a different library without confirming. Prophet's holidays are pre-configured and align with the WW14 narrative.
...
- Model scorecard CSV
- Lag analysis CSV (mirrors WW14 format)
